...
object CheckIsEmpty extends SimplificationType {
  ...
  def unapply(expr: ScExpression): Option[(ScExpression, Int, Int)] = {
    val firstLevel = expr match {
      case (coll`.sizeOrLength` Seq()) `==` literal("0") => Some((coll, coll.end, expr.end))
      case coll`.isEmpty` Seq() => Some((coll, coll.end, expr.end))
  ...
  def extractInner(firstLevel: Option[(ScExpression, Int, Int)]): Option[(ScExpression, Int, Int)] = {
    firstLevel match {
      case None => None
      case Some((inner @ coll`.headOption` Seq(), start, end)) =>
        Some(coll, Math.min(coll.end, start), Math.max(inner.end, end))
      case Some((inner @ coll`.lastOption` Seq(), start, end)) =>
        Some(coll, Math.min(coll.end, start), Math.max(inner.end, end))
      case _ => firstLevel
    }
  }
...
object CheckNonEmpty extends SimplificationType {
  ...
  def unapply(expr: ScExpression): Option[(ScExpression, Int, Int)] = {
    val firstLevel = expr match {
      case qual`.nonEmpty`Seq() => Some((qual, qual.end, expr.end))
      case (qual`.sizeOrLength`Seq()) `!=` literal("0") => Some((qual, qual.end, expr.end))
      case (qual`.sizeOrLength`Seq()) `>` literal("0") => Some((qual, qual.end, expr.end))
      case (qual`.sizeOrLength`Seq()) `>=` literal("1") => Some((qual, qual.end, expr.end))
      ...
      case qual`.isDefined` Seq() if isOption(qual) => Some(qual, qual.end, expr.end)
      case _ => None
    }
